...
grid_forge_core::__impl_tile! {
    ...
    container_trait: TileContainer2D,
}

pub trait TileContainer2D {
...
impl<Data: TileData> TileContainer2D for PosData2D<Data> {
    fn grid_position(&self) -> GridPosition2D {
        self.0
    }
}
...
impl<Data: TileData> TileContainer2D for PosDataRef2D<'_, Data> {
    fn grid_position(&self) -> GridPosition2D {
        self.0
    }
}
...
impl<Data: TileData> TileContainer2D for PosDataMutRef2D<'_, Data> {
    fn grid_position(&self) -> GridPosition2D {
        self.0
    }
}
