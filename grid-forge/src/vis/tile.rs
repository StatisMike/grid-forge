...
use crate::{id::SharedData, TileData};

use super::{PixelWithDefault};
...
pub trait TilePixels<P: PixelWithDefault>: Clone {
...
impl<const WIDTH: usize, const HEIGHT: usize, P: PixelWithDefault> TilePixConst<WIDTH, HEIGHT, P> {
    pub fn from_slice(pixels: &[P]) -> Self {
        match pixels.chunks_exact(WIDTH).map(|chunk| chunk.try_into().unwrap()).collect::<Vec<_>>().try_into() {
...
impl<const WIDTH: usize, const HEIGHT: usize, P: PixelWithDefault> TilePixels<P> for TilePixConst<WIDTH, HEIGHT, P> {
    ...
    fn new_empty(_width: usize, _height: usize) -> Self {
        Self { pixels: [[P::pix_default(); WIDTH]; HEIGHT] }
    }
}

impl <const WIDTH: usize, const HEIGHT: usize, P: PixelWithDefault> TileData for TilePixConst<WIDTH, HEIGHT, P> {}
impl <const WIDTH: usize, const HEIGHT: usize, P: PixelWithDefault> SharedData for TilePixConst<WIDTH, HEIGHT, P> {}
...
impl<P: PixelWithDefault> TilePixVar<P> {
    pub fn from_slice(pixels: &[P], width: usize, height: usize) -> Self {
        Self { width, height, pixels: pixels.to_vec() }
    }
}
...
impl<P: PixelWithDefault> TilePixels<P> for TilePixVar<P> {
    ...
    fn new_empty(width: usize, height: usize) -> Self {
        Self { width, height, pixels: vec![P::pix_default(); width * height] }
    }
}
