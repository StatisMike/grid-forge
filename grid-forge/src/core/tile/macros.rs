macro_rules! __impl_tile {
    (
        ...
        use crate::{TileData, TileContainer};
        use crate::id::{SharedData, WithSharedData};
        ...
        impl<D: TileData> AsRef<D> for $tile_type<D> {
            fn as_ref(&self) -> &D {
                &self.1
            }
        }

        impl<D: TileData> TileContainer<$position, D> for $tile_type<D> {
            fn grid_position(&self) -> $position {
                self.0
            }
        }

        impl<D: TileData> Sealed for $tile_type<D> {}
        ...
        impl<'a , D: TileData> TileContainer<$position, D> for $tile_ref_type<'a, D> {
            fn grid_position(&self) -> $position {
                self.0
            }
        }
        ...
        impl<'a , D: TileData> TileContainer<$position, D> for $tile_mut_type<'a, D> {
            fn grid_position(&self) -> $position {
                self.0
            }
        }
        ...
        impl<'a , D: TileData, Shared: SharedData> TileContainer<$position, D> for $tile_ref_shared_type<'a, D, Shared> {
            fn grid_position(&self) -> $position {
                self.0
            }
        }
        ...
        impl<'a , D: TileData, Shared: SharedData> TileContainer<$position, D> for $tile_mut_shared_type<'a, D, Shared> {
            fn grid_position(&self) -> $position {
                self.0
            }
        }
